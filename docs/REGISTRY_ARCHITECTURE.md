# Архитектура реестров PKCS#12 и общие требования для разработки сервисов работы с форматом **ATOM-PKCS12-REGISTRY**,

Документ описывает архитектуру реестров в формате **ATOM-PKCS12-REGISTRY**, формальную спецификацию формата на основе [registry.asn1](../registry.asn1) и требования при разработке сервисов работы с реестрами. В разделах документа **есть ссылки на утилиты проекта** (registry-analyzer, registry-builder, p7-analyzer) и используются **как рекомендации для разработки собственных сервисов** работы с реестрами формата ATOM-PKCS12-REGISTRY.
...
flowchart TB
  ...
  subgraph CMS ["Блок CMS (PKCS#7 / RFC 5652)"]
    ...
    SD --> certs["certificates:[Сertificate storage 1]"]
    ...
    SC --> SB["SafeBag:[Certificate storage 2]"]
...
flowchart LR
  subgraph SignedData ["SignedData (CMS)"]
    M1["Certificate storage 1: certificates"]
    M2["Certificate storage 2: eContent -> SafeContents -> SafeBag"]
  end
  M1 --> A["Подписант + цепочка CA"]
  M2 --> B["Сертификаты ролей (по одному в SafeBag)"]
```

### Схема уровней структуры контейнера 
...
+-- PFX  (version = 3)  <-- PKCS#12 (оболочка)
    ...
    |   |
    |   |  === Начало блока CMS (PKCS#7, RFC 5652). ================
    |   |    блок: ContentInfo, SignedData, eContent->SafeBag, certs.
    |   |  =========================================================
    ...
    |               |               |
    |               |               |  [Certificate storage 2 : сертификаты ролей (Driver, IVI)]
    ...
    |               |
    |               |  [Certificate storage 2: сертификаты подписанта и CA (не в SafeBag)]
...
### Два источника сертификатов (не только SafeBag в .p12)

**Примечание :** *не все данные сертификатов лежат внутри SafeBag.* В CMS (PKCS#7) реестра сертификаты находятся в **двух разных местах** (см. диаграмму двух источников выше):
...
**SafeBags** —  используются в контейнере **.p12** (ATOM-PKCS12-REGISTRY). В файлах **.p7** (<<чистый>> CMS без обёртки PFX) eContent может быть в другом формате (например PEM-цепочки, не SafeBags).

### Связь с утилитами проекта

Для формирования требований к разработке сервисов работы с форматом реестров (**ATOM-PKCS12-REGISTRY**), были разработаны утилиты работы с этим типом контейенра.
...
| **Два источника сертификатов** | Различать SignedData.certificates (сертификаты для подписи/доверия) и eContent (SafeContents) — сертификаты ролей. Оба слоя обрабатывать и при необходимости экспортировать раздельно. |
