...
Для реестра, подписанного сертификатом от тестового корня: выгрузите подписанта (`./registry-analyzer -export-signer-cert sgw-owner-6-new.p12`), затем проверьте цепочку с **certs/root-ca.pem**. Подробно — в [OPENSSL_VERIFY.md](OPENSSL_VERIFY.md) (раздел «Проверка цепочки доверия на основе тестовых сертификатов (root-ca.pem)»).
